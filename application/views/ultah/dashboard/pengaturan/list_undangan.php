                    ...
                    <div class="row">
                        <div class="col-lg">
                            <div class="p-1">
                                            ...
                                            <tbody>
                                                ...
                                                <?php foreach ($undangan as $l) : ?>
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <a href="<?= base_url(); ?>DashboardUltah/edit_tamu/<?= $l['id']; ?>" class="badge rounded-pill bg-warning">Edit</a>
                                                            <a href="<?= base_url(); ?>DashboardUltah/hapus_tamu/<?= $l['id']; ?>" class="badge rounded-pill bg-danger" onclick="return confirm('Apa anda yakin?');">Hapus</a>
                                                        </td>
                                ...
                                <?php endif; ?>
                                <form class="user" action="<?= base_url('DashboardUltah/tambah_list'); ?>" method="POST" enctype="multipart/form-data">
                                    <div class="card shadow mb-4">
                                        <div class="card-header py-3">
...
<script>
    function sendWa(id, hp, nm) {
        $.ajax({
            url: "<?= base_url() . 'DashboardUltah/update_status_kirim_wa?id=' ?>" + id,
            type: "GET",
        });
        let pesan = $('#pesan').val();
        let url = "<?= base_url() ?>";
        pesan += url;
        pesan += 'UUlangtahun?to=' + nm;
